...
# can convert color to root type color by TColor::GetColor
...
class ProcessMap:
    ...
    background_groups: Optional[Dict[str, List[str]]] = None
    background_colors: Optional[Dict[str, str]]= None

    @classmethod
    def from_csv(cls, csv_path: Optional[str] = None):
        """
        ...
        data = list(grp_dict.get("Data")) if grp_dict.get("Data") else None
        background_groups = {
            k: list(v) for k, v in grp_dict.items() if k not in ("Signal", "Data") and v 
        }
