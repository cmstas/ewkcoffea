...
categories = ["sig"]
years = ["run3"]
input_json_template = "config/run3_sig.json"
output_csv = "run3_sample_names.csv"
...
def run_makeConfig():
    for cat in categories:
        for year in years:
            print(
                f"Running: python3 etc/makeConfig.py --categories {cat} --sample_year {year}"
            )
            subprocess.run(
                [
                    "python3",
        ...
        for year in years:
            ...
            for sample_key, sample_info in samples.items():
                ...
                if sample_category == "sig":
                    matched_type = next(
                        (tag for tag in signal_tags if tag in sample_key), "unknown_sig"
                    )
...
def write_sample_code_map(sample_list, output_path="sample_code.csv"):
    ...
    with open(output_path, "w", newline="") as csvfile:
        writer = csv.DictWriter(
            csvfile,
            fieldnames=["sample_code", "sample_category", "sample_type", "sample_name"],
        )
        writer.writeheader()
