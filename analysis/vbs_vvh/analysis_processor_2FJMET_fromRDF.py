...
from config.paths import cutflow_yamls_dir, default_cutflow_yaml, get_cutflow, objsel_cf
from config.variables.extra_definitions import Higgs_final_state, VV_ndaughters
...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(
        self, samples, wc_names_lst=[], n_minus_1=False, project=None, cutflow_name=None, run=2,
    ):
        print("debug", project, cutflow_name)

        if run==2:
            print('importing run2 obj')
            from config.variables.var_config import obj,other_objs, dense_variables_config
        elif run==3:
            print('importing run3 obj')
            from config.variables.run3_var_config import dense_variables_config, obj, other_objs
        else:
            print('run number value error should have been handled in run_analysis. why? run is',run)
            import sys
            sys.exit(1)

        self.obj = obj
        self.other_objs = other_objs
        self.dense_variables_config = dense_variables_config
        ...
        if cutflow_name is None:
            ...
            try:  # try project, then try all.yaml, finally just use basic
                self.cutflow = get_cutflow(
                    f"{cutflow_yamls_dir}/{project}.yaml", cutflow_name
                )
            except:
                try:
                    self.cutflow = get_cutflow(default_cutflow_yaml, cutflow_name)
                    print(
                        f"Cannot get {cutflow_name} in {str(project)}.yaml. Found cutflow in defualt yaml and use it instead"
                    )
                except:
                    try:
                        print(get_cutflow(f"{cutflow_yamls_dir}/{project}.yaml"))
                        print(
                            "can get project yaml but not the cutflow. Use default cutflow instead."
                        )
                        self.cutflow = objsel_cf
                    except:
                        self.cutflow = objsel_cf
                        print(
                            f"Warning: Cutflow {cutflow_name} not found. Use default cutflow instead."
                        )
            else:
        ...
        self._dtype = (
            np.float32
        )  # note that this has been changed from function input to this fixed defintion

        # Create the dense axes for the histograms
        self._dense_axes_dict = {
            var_name: cfg["axis"] for var_name, cfg in self.dense_variables_config.items()
        }
        ...
        for dense_axis_name in self._dense_axes_dict.keys():
            dout[dense_axis_name] = hist.Hist(
                ...
                hist.axis.StrCategory(
                    [], growth=True, name="category", label="category"
                ),
                hist.axis.StrCategory(
                    [], growth=True, name="systematic", label="systematic"
                ),
    ...
    def process(self, events):
        ...
        objects = {name: builder(events) for name, builder in self.obj.items()}
        objects.update(
            {name: builder(events, objects) for name, builder in self.other_objs.items()}
        )
        ...
        var = {
            ...
            for var_name, cfg in self.dense_variables_config.items()
        }
        # Dataset parameters
        ...
        #events["nom"] = ak.ones_like(events.weight)
        ...
        for sel, crit_fn in self.cutflow.items():
            mask = crit_fn(events, var, self.obj)
        ...
        if isSig:  # or use the ak.firsts trick
            ...
            # wgt_var_dict["SM"] = events.weight * ak.fill_none(
            #     events.LHEReweightingWeight[:, 60], 0
            # )
            # wgt_var_dict["c2v_1p5"] = events.weight * ak.fill_none(
            #     events.LHEReweightingWeight[:, 72], 0
            # )
            ...
            # wgt_var_dict["c2v_1p5_qqNuNu"] = (
            #     events.weight
            #     * ak.fill_none(events.LHEReweightingWeight[:, 72], 0)
            #     * mask1
            # )
            # wgt_var_dict["c2v_1p5_qqlNu"] = (
            #     events.weight
            #     * ak.fill_none(events.LHEReweightingWeight[:, 72], 0)
            #     * mask2
            # )

        if not isSig and not isData:
            # then it is bkg
            # wgt_var_dict["MET_rewc"] = events.reweighting_weight
            # wgt_var_dict["MET_rewc_qcd"] = events.reweighting_weight_qcd
            wgt_var_dict["MET_rewc"] = events.reweight_weighting
            # add other cp
        # -----------------------------------------------------------
        ...
        if self._n_minus_1:
            ...
            for sel in self.cutflow:
                if "objsel" in self.cutflow:
                    exclusive_sels = [k for k in self.cutflow if k != sel] + [
                        "objsel"
                    ]  # + list(objsel_cf)
                else:
                    exclusive_sels = [k for k in self.cutflow if k != sel] + list(
                        objsel_cf
                    )
            ...
            for sel, sel_mask in cut_mask_dict.items():
                for dense_axis_name, dense_axis_vals in dense_variables_dict.items():
                    axes_fill_info_dict = {
                        ...
                        "process": ak.fill_none(
                            events.name[sel_mask], "unknown"
                        ),  # changed from sample_name to name
                        "category": sel,
                        "systematic": wgt_key,
                    }
                    # Include year if needed (can be safely added here)
                    if "year" in self.accumulator[dense_axis_name].axes.name:
                        axes_fill_info_dict["year"] = ak.fill_none(
                            events.year[sel_mask], "unknown"
                        )  # changed from sample_year to year
