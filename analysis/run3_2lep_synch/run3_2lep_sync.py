...
import ewkcoffea.modules.selection_Run3_2Lep_SYNC as selRun3_2Lep
import ewkcoffea.modules.corrections as ewk_corrections
import ewkcoffea.modules.objects_Run3_2Lep_SYNC as objRun3_2Lep
...
#            pileup = events.Pileup
        ...
        ele_mask = objRun3_2Lep.is_tight_Run3_2Lep_ele(ele)
        ele["is_tight_lep_for_Run3_2Lep"] = (ele_mask)
        ele_Run3_2Lep_tight = ele[ele.is_tight_lep_for_Run3_2Lep]
        ele_Run3_2Lep_tight = ele_Run3_2Lep_tight[ak.argsort(ele_Run3_2Lep_tight.pt, axis=-1,ascending=False)] 

        # Grab the pre-selected muons and sort by PT
        mu_mask = objRun3_2Lep.is_tight_Run3_2Lep_mu(mu)
        mu["is_tight_lep_for_Run3_2Lep"] = (mu_mask)
        mu_Run3_2Lep_tight = mu[mu.is_tight_lep_for_Run3_2Lep]
        mu_Run3_2Lep_tight = mu_Run3_2Lep_tight[ak.argsort(mu_Run3_2Lep_tight.pt, axis=-1,ascending=False)] 

        #Attach the SF
        #ewk_corrections.run3_muons_sf_Attach(mu_Run3_2Lep_veto,year,"NUM_MediumID_DEN_TrackerMuons","NUM_TightPFIso_DEN_MediumID")
        #ewk_corrections.run3_electrons_sf_Attach(ele_Run3_2Lep_veto,year,"wp80iso")
        ...
        l_Run3_2Lep_tight = l_Run3_2Lep_tight[ak.argsort(l_Run3_2Lep_tight.pt, axis=-1,ascending=False)] 
        ...
        jets_Run3_2Lep = jets[ak.argsort(jets_Run3_2Lep.pt, axis=-1,ascending=False)] 
        ...
        for syst_var in syst_var_list:
            ...
            selRun3_2Lep.attach_Run3_2Lep_preselection_mask(events,l_Run3_2Lep_tight_padded[:,0:2])
            selRun3_2Lep.addMuTriggerMask(events)                                             
            selRun3_2Lep.addEleTriggerMask(events)                                             
            selections = PackedSelection(dtype='uint64')

            # For Run3 2Lep selection
            selections.add("2l_sf_ee", (events.is2l & events.Run3_2Lep_presel_sf_ee & events.EleTrigMask))
            selections.add("2l_sf_mumu", (events.is2l & events.Run3_2Lep_presel_sf_mumu & events.MuTrigMask))

            sr_cat_dict = {
                "lep_chan_lst" : ["2l_sf_mumu", "2l_sf_ee"]
            }
